...
func TestFreeListLengthOne(t *testing.T) {
	...
	require.NoError(t, fl.Put(context.Background(), ptrutil.ToPtr(0)))
...
func TestFreeListMultiple(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		require.NoError(t, fl.Put(context.Background(), ptrutil.ToPtr(i)))
	}
...
func TestFreeListCancellation(t *testing.T) {
	...
	ctx, cancel = context.WithCancel(context.Background())
	require.NoError(t, fl.Put(ctx, ptrutil.ToPtr(0)))
	...
	err = fl.Put(ctx, ptrutil.ToPtr(1))
	require.ErrorIs(t, context.Canceled, err)
}
	...
	for _, bench := range benches {
		b.Run(fmt.Sprintf("%d-%d", bench.workers, bench.objects), func(b *testing.B) {
			...
			for i := 0; i < bench.objects; i++ {
				fl.Put(context.Background(), ptrutil.ToPtr(0)) //nolint:errcheck
			}
